...
import { Container, Row, Col, Image, Table, ResponsiveEmbed, Carousel } from 'react-bootstrap';
...
const BlogPage = ({ match }) => {
  ...
  return (
    <Container className="page-container">
      ...
      <hr className={styles.sectionDivider} />
      {
        headerImage ? renderBlogPageHeader(
          <div className="text-center"><Image src={headerImage} fluid rounded className={styles.headerImage} /></div>
        ) : ''
      }
      {
        headerImages ? renderBlogPageHeader(
          <Carousel>
            {
        ...
        <Col>
          {
            body.map(section => {
              switch(section.type) {
                ...
                case 'image':
                  return (<div className="text-center"><Image fluid rounded src={section.image} className={styles.blogImage} /></div>);
                case 'heading':
                ...
                case 'author':
                  return (
                    <div className="d-flex flex-column">
                      ...
                      {section.biography ? <p className="font-italic">{section.biography}</p> : ''}
                    </div>
                  );
                case 'list':
                  if (section.ordered) {
                    return (
                      <ol>{section.data.map(item => <li>{item}</li>)}</ol>
                    )
                  } else {
                    return (
                      <ul>{section.data.map(item => <li>{item}</li>)}</ul>
                    )
                  }
                case 'table':
                  return (
                    <React.Fragment>
                      ...
                      <Table striped bordered hover>
                        <thead>
                          <tr>
                            { section.column_headers.map(header => <th>{header}</th>) }
                          </tr>
                        </thead>
                        <tbody>
                          {
